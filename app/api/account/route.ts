...
export async function GET() {
  ...
  const db = getDb();
  const [profileRows, conversationRows, messageRows, memoryRows] = await Promise.all([
  ...
  const payload = JSON.stringify({ exportedAt: new Date().toISOString(), profile: profileRows[0] ?? null, conversations: conversationRows, messages: messageRows, memories: memoryRows }, null, 2);
