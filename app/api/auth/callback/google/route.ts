...
export async function GET(request: Request) {
  ...
  try {
    ...
    const response = Response.redirect(`${url.origin}${safeRelativeReturnPath(decodedReturn)}`, 302);
...
function loginError(origin: string, message: string) { return Response.redirect(`${origin}/login?error=${encodeURIComponent(message)}`, 302); }
