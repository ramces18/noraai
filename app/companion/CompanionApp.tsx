...
function PetVisual({ companion, state }: { companion: Companion; state: PetState }) {
  ...
  return <svg className={`pet-visual pet-${companion.petType} pet-${state}`} viewBox="0 0 280 270" role="img" aria-label={`${companion.name}, tu compañero ${PET_TYPES.find(item => item.value === companion.petType)?.label.toLowerCase() ?? "virtual"}`}>
    ...
    <g className="pet-tail" fill="none" stroke={palette.color} strokeWidth={isDog ? 25 : 22} strokeLinecap="round"><path d={isDog ? "M205 191 Q263 174 235 132" : isRabbit ? "M205 188 Q256 204 239 155" : "M205 188 Q267 206 245 143"}/></g>
    <g className="pet-body">
      <ellipse className="pet-torso" cx="140" cy="184" rx="77" ry="58" fill={palette.color}/>
      <circle className="pet-head" cx="140" cy={faceY} r="70" fill={palette.color}/>
      {isRabbit ? <><path className="pet-ear" d="M99 53 Q77 -35 111 3 L130 48Z" fill={palette.color}/><path className="pet-ear-inner" d="M102 42 Q92 -8 108 14 L119 42Z" fill={palette.accent}/><path className="pet-ear" d="M151 47 Q157 -34 181 8 L168 58Z" fill={palette.color}/><path className="pet-ear-inner" d="M158 42 Q164 -7 174 14 L166 45Z" fill={palette.accent}/></> : isDog ? <><path className="pet-ear" d="M94 63 Q44 47 57 124 L100 108Z" fill={palette.color}/><path className="pet-ear-inner" d="M82 72 Q59 67 68 108 L91 99Z" fill={palette.accent}/><path className="pet-ear" d="M186 62 Q237 45 222 123 L180 107Z" fill={palette.color}/><path className="pet-ear-inner" d="M198 72 Q220 66 211 108 L188 98Z" fill={palette.accent}/></> : <><path className="pet-ear" d="M89 53 L93 5 L131 38Z" fill={palette.color}/><path className="pet-ear-inner" d="M99 39 L101 19 L117 37Z" fill={palette.accent}/><path className="pet-ear" d="M149 37 L190 4 L187 65Z" fill={palette.color}/><path className="pet-ear-inner" d="M161 37 L179 20 L178 49Z" fill={palette.accent}/></>}
    ...
    <g className="pet-face" fill={palette.accent} transform={`translate(0 ${isRabbit ? 5 : 0})`}>
      ...
      <ellipse className="pet-muzzle" cx="140" cy="111" rx={isDog ? 23 : 17} ry="14" fill={palette.accent} opacity=".78"/>
      <path className="pet-nose" d="M134 108 Q140 103 146 108 Q140 115 134 108Z" fill={palette.color}/>
