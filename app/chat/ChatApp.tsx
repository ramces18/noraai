...
export default function ChatApp({ user, googleWelcome = false }: Props) {
  ...
  return <div className="chat-shell">
    ...
    <aside className={sidebar ? "sidebar open" : "sidebar"} aria-label="Historial de conversaciones">
      ...
      <div className="side-links"><a href="/companion"><span>♡</span> Mi compañero</a><button type="button" onClick={() => { setSidebar(false); void showSettings("memory"); }}><span>◇</span> Recuerdos</button><button type="button" onClick={() => { setSidebar(false); void showSettings("privacy"); }}><span>⌁</span> Privacidad</button></div>
